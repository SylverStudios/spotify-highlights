...
```bash
spotify-highlights$ sam build --use-container
```
...
```bash
spotify-highlights$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
spotify-highlights$ sam local start-api
spotify-highlights$ curl http://localhost:3000/
```
...
```bash
spotify-highlights$ sam logs -n HelloWorldFunction --stack-name spotify-highlights --tail
```
...
```bash
spotify-highlights$ pip install pytest pytest-mock --user
spotify-highlights$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name spotify-highlights
```
